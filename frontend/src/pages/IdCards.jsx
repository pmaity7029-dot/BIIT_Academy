...
import { FiCamera, FiEdit3, FiPlus, FiDownload, FiSearch, FiTrash2, FiUser } from 'react-icons/fi'; // FiPrinter removed
import dayjs from 'dayjs';
import html2canvas from 'html2canvas'; // For JPG Download
...
export default function IdCards() {
  ...
  const handleSelectStudentForEditor = (studentId) => {
    ...
    if (student) {
      setEditorData((prev) => ({
        ...
        regNo: student.regNo,
        batch: student.batch || 'Default Batch',
  ...
  // Direct Frontend Cloudinary Upload Logic
  const customCloudinaryUpload = async ({ file, onSuccess, onError }) => {
    if (!CLOUD_NAME || !UPLOAD_PRESET) {
      message.error("Cloudinary configurations are missing in .env file!");
      onError(new Error("Missing Config"));
      return;
    }
  ...
  const openNewIdCardEditor = () => {
    setEditingId(null);
    setEditorData(defaultCardValues);
    setEditorOpen(true);
  };

  const openEditIdCardEditor = (record) => {
  ...
  const saveIdCardRecord = async () => {
    if (!editorData.studentName || !editorData.regNo || !editorData.phone || !editorData.dob) {
      message.error('Student Name, Reg No, Phone, and DOB are required!');
      return;
    }
    ...
    try {
      ...
      if (payload.dob && typeof payload.dob.toISOString === 'function') {
        payload.dob = payload.dob.toISOString();
      } else if (payload.dob) {
        payload.dob = new Date(payload.dob).toISOString();
      }

      if (!payload.student || payload.student === '') {
  ...
  // NAYA FUNCTION: High-Quality JPG Generator (html2canvas)
  const downloadIdCard = async () => {
    ...
    try {
      ...
      // Sabhi images load hone ka wait karein (especially Cloudinary profile photo)
      ...
      const canvas = await html2canvas(element, {
        scale: 4, // 4x Scale taaki text quality perfect rahe (Print-ready)
        useCORS: true,
        allowTaint: true,
  ...
  return (
    <div>
      <PageHeader
        ...
        actionText="Generate ID Card"
        actionIcon={<FiPlus />}
        onAction={openNewIdCardEditor}
      />
        ...
        <Card className="content-card" bordered={false}>
          <div className="section-toolbar">
            ...
            <Space wrap>
              ...
              {/* FIXED: Print Icon ko Download Icon se replace kiya hai */}
        ...
        <div className="section-toolbar compact-toolbar">
          <Space wrap>
            <Input.Search
              ...
              placeholder="Search by student name, reg no, batch..."
              value={search}
              onChange={(e) => {
                setSearch(e.target.value);
                loadIdCards(e.target.value);
              }}
              onSearch={(val) => {
                setSearch(val);
                loadIdCards(val);
      ...
      {/* FIXED: Modal property destroyOnClose updated to destroyOnHidden */}
      <Modal
        title={editingId ? "Edit Identity Card Records" : "Generate Custom New Student ID Card"}
        open={editorOpen}
        onCancel={() => setEditorOpen(false)}
        onOk={saveIdCardRecord}
        okText={editingId ? "Update Data" : "Save Record to Database"}
        width={850}
        destroyOnHidden
      >
              ...
              {!editingId && (
                <>
                  ...
                  <Select
                    ...
                    placeholder="Optional: Select student profile"
                    optionFilterProp="label"
                    onChange={handleSelectStudentForEditor}
          ...
          <Col xs={24} md={12} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f8fafc', borderRadius: '12px' }}>
            <div className="id-card-wrapper">
              ...
              <div className="id-card-body">
                <div className="id-photo-box">
                  {editorData.photo ? (
                    <img src={editorData.photo} alt="Preview" crossOrigin="anonymous" />
                  ) : (
                    <div className="id-photo-placeholder"><FiUser /></div>
                  )}
                </div>
                ...
                <div className="id-details">
                  <div className="id-detail-row">
                    <span>Reg No:</span> <strong>{editorData.regNo || 'BIITXXXXXX'}</strong>
                  </div>
                  <div className="id-detail-row">
                    <span>DOB:</span> <strong>{editorData.dob ? editorData.dob.format('DD/MM/YYYY') : 'DD/MM/YYYY'}</strong>
                  </div>
                  <div className="id-detail-row">
                    <span>Blood:</span> <strong>{editorData.bloodGroup}</strong>
                  </div>
                  <div className="id-detail-row">
                    <span>Phone:</span> <strong>{editorData.phone || '0000000000'}</strong>
                  </div>
                  <div className="id-detail-row" style={{ borderBottom: 'none' }}>
                    <span>Valid Till:</span> <strong>{editorData.validUntil}</strong>
                  </div>
                </div>
              </div>
              <div className="id-card-footer">
