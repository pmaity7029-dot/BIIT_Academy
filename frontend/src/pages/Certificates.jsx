...
const calculateMarksSummary = (rows = []) => {
  const totalFullMarks = rows.reduce((sum, row) => sum + getMarksNumber(row.fullMarks), 0);
  const totalMarksObtain = rows.reduce((sum, row) => sum + getMarksNumber(row.marksObtain), 0);
...
const withCalculatedMarks = (certificate) => {
  ...
  return { ...certificate, moduleRows, ...calculateMarksSummary(moduleRows) };
};
...
const getCertificateData = (certificate) => {
  ...
  return withCalculatedMarks({
    ...
    issueDateText: certificate.issueDateText || (certificate.issueDate ? dayjs(certificate.issueDate).format('DD/MM/YY') : dayjs().format('DD/MM/YY')),
...
const CERTIFICATE_STYLES = `
  .classic-certificate-wrap { position: relative; z-index: 0; isolation: isolate; width: 100%; overflow-x: auto; padding: 14px 0; }
  .classic-certificate { position: relative; z-index: 0; isolation: isolate; width: 1122px; height: 794px; margin: 0 auto; overflow: hidden; border: 2px solid #7d5a1d; color: #050505; background: radial-gradient(circle at 51% 48%, rgba(255, 253, 237, 0.98) 0%, rgba(255, 246, 202, 0.98) 38%, rgba(237, 204, 102, 0.96) 68%, #c89231 100%); font-family: Georgia, "Times New Roman", serif; box-shadow: 0 24px 70px rgba(80, 45, 0, 0.22); -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  .classic-certificate, .classic-certificate * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  .classic-bg-pattern { position: absolute; top: -150px; left: 112px; z-index: 1; width: 1120px; height: 1040px; display: grid; grid-template-columns: repeat(3, 360px); grid-auto-rows: 27px; column-gap: 16px; row-gap: 0; overflow: hidden; transform: rotate(-13deg); transform-origin: center; opacity: 0.16; color: #9b741e; font-family: Georgia, "Times New Roman", serif; font-size: 20px; line-height: 1; font-weight: 700; white-space: nowrap; pointer-events: none; }
  .classic-bg-pattern span { display: block; }
  .classic-left-curve { position: absolute; left: -168px; top: -74px; z-index: 2; width: 420px; height: 950px; border-radius: 0 58% 58% 0; background: linear-gradient(105deg, #8a651f 0%, #a87a23 43%, #c59636 55%, rgba(255, 229, 150, 0.42) 61%, rgba(255,255,255,0) 72%); box-shadow: 24px 0 0 rgba(140, 102, 24, 0.18); transform: rotate(7deg); pointer-events: none; }
  .classic-left-curve::after { content: ''; position: absolute; right: 56px; top: 0; width: 28px; height: 100%; border-radius: 50%; background: rgba(255, 223, 128, 0.28); }
  .classic-logo-watermark { position: absolute; z-index: 3; left: 438px; top: 278px; width: 255px; opacity: 0.035; filter: grayscale(10%); pointer-events: none; }
  .classic-layer { position: relative; z-index: 20; width: 100%; height: 100%; padding: 24px 30px 0; }
  .classic-logo { position: absolute; left: 22px; top: 82px; z-index: 25; width: 175px; height: 166px; display: grid; place-items: center; }
  .classic-logo img { width: 178px; height: 158px; object-fit: contain; filter: drop-shadow(0 8px 10px rgba(0, 0, 0, 0.28)); }
  .classic-top { padding-left: 170px; text-align: center; }
  .classic-institute { margin: 0; color: #ffffff; font-size: 43px; line-height: 0.98; font-variant: small-caps; letter-spacing: 1.3px; text-shadow: 2px 3px 4px rgba(0,0,0,0.58); }
  .classic-subtitle { margin: 8px 0 0; color: #1a1a1a; font-family: Arial, sans-serif; font-size: 20px; line-height: 1.08; font-weight: 500; }
  .classic-foundation { margin: 4px 0 0; color: #9f2727; font-family: Arial, sans-serif; font-size: 21px; line-height: 1.08; font-weight: 800; }
  ...
  .classic-accreditation-label, .classic-initiative-label, .classic-partner-label { color: #111; font-size: 14px; font-weight: 700; margin: 7px 0 2px; }
  .classic-partner-label { font-size: 15px; margin-top: 1px; }
  .classic-training-img { width: 142px; height: 48px; object-fit: contain; display: block; }
  .classic-accreditation-grid { display: grid; grid-template-columns: 122px 158px; gap: 7px 10px; align-items: center; }
  .classic-cert-img-box { height: 56px; display: flex; align-items: center; justify-content: center; padding: 2px; border: 1px solid rgba(70, 55, 28, 0.65); background: rgba(255, 255, 255, 0.78); }
  .classic-cert-img-box img { max-width: 100%; max-height: 100%; object-fit: contain; display: block; }
  .classic-cert-img-box.tall { height: 58px; }
  .classic-cert-img-box.nsdc { height: 58px; width: 158px; }
  .classic-signatures { display: grid; grid-template-columns: repeat(3, 1fr); gap: 24px; margin-top: 30px; padding-right: 4px; color: #111; font-size: 16px; font-weight: 900; text-align: center; }
  .classic-signatures span { border-top: 1px solid rgba(0,0,0,0.25); padding-top: 5px; }
  .classic-module-table { width: 100%; border-collapse: collapse; background: rgba(255,255,255,0.86); color: #000; font-size: 14px; line-height: 1.05; box-shadow: 0 2px 8px rgba(0,0,0,0.12); }
  .classic-module-table th, .classic-module-table td { border: 1px solid #000; padding: 5px 7px; vertical-align: middle; }
  .classic-module-table th { background: rgba(255,255,255,0.95); font-weight: 900; text-align: center; }
  .classic-module-table td:first-child { width: 28px; font-weight: 900; text-align: center; }
  .classic-module-table td:nth-child(3), .classic-module-table td:nth-child(4) { width: 74px; text-align: center; }
  .classic-total-row td { background: rgba(255,255,255,0.96); font-size: 16px; font-weight: 900; }
  .classic-issue { margin-top: 22px; color: #111; font-size: 16px; font-weight: 900; text-align: center; }
  .classic-footer-strip { position: absolute; left: 0; right: 0; bottom: 34px; z-index: 30; height: 40px; display: flex; align-items: center; justify-content: space-between; padding: 0 30px; background: #a9791f; color: #ffffff; font-size: 17px; font-weight: 900; letter-spacing: 0.02em; }
  .classic-grade-note { position: absolute; left: 58px; right: 290px; bottom: 8px; z-index: 30; color: #df1f1f; font-family: Arial, sans-serif; font-size: 13px; font-weight: 800; background: rgba(255, 255, 225, 0.72); }
  .classic-verify-note { position: absolute; right: 12px; bottom: 8px; z-index: 30; color: #4a3a1b; font-family: Arial, sans-serif; font-size: 10px; font-weight: 800; }
  .editable-cert-field { min-width: 22px; display: inline-block; border-radius: 4px; outline: 2px dashed rgba(20, 63, 117, 0.55); outline-offset: 2px; cursor: text; }
  .editable-cert-field:focus { background: #fff6a3; outline: 2px solid #143f75; }
  .certificate-editor-grid { display: grid; grid-template-columns: 360px 1fr; gap: 18px; }
  .certificate-editor-form { max-height: 76vh; overflow: auto; padding-right: 6px; }
  .editor-field-label { display: block; margin: 12px 0 5px; color: #53647d; font-size: 12px; font-weight: 800; text-transform: uppercase; }
  .editor-hint { margin-bottom: 12px; padding: 10px 12px; border: 1px dashed #d4af37; border-radius: 12px; background: #fffbea; color: #6b560f; font-size: 13px; font-weight: 700; }
  .module-editor-row { display: grid; grid-template-columns: 46px 1fr 70px 70px 36px; gap: 6px; margin-bottom: 8px; }
  @media (max-width: 1200px) {
    .classic-certificate { transform: scale(0.84); transform-origin: top left; margin-bottom: -115px; }
    .classic-certificate-wrap { height: 690px; }
...
const printStyles = `
  ...
  html, body { width: 297mm; height: 210mm; margin: 0 !important; padding: 0 !important; overflow: hidden; background: #ffffff; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
  .print-stage { width: 297mm; height: 210mm; display: grid; place-items: center; background: #ffffff; }
  .classic-certificate-wrap { width: 297mm !important; height: 210mm !important; padding: 0 !important; overflow: hidden !important; }
  .classic-certificate { width: 297mm !important; height: 210mm !important; margin: 0 !important; border: 0 !important; box-shadow: none !important; transform: none !important; }
  .editable-cert-field { outline: 0 !important; }
`;
  ...
  return (
    <div ref={ref} className="classic-certificate">
      ...
      <div className="classic-layer">
        ...
        <div className="classic-body">
          <p className="classic-main-text">
            ...
            <EditableField value={data.studentName} editable={editable} className="classic-highlight" onChange={edit('studentName')} />
            , {getRelation(data.gender)} of{' '}
            <EditableField value={data.fatherName} editable={editable} className="classic-highlight" onChange={edit('fatherName')} />
            , date of birth{' '}
            <EditableField value={data.birthDateText} editable={editable} className="classic-highlight" onChange={edit('birthDateText')} />
            , on completion of{' '}
            <EditableField value={data.courseTitle} editable={editable} className="classic-highlight" onChange={edit('courseTitle')} />{' '}
            duration of{' '}
            <EditableField value={data.duration} editable={editable} className="classic-highlight" onChange={edit('duration')} />{' '}
            with grade{' '}
            <EditableField value={data.grade} editable={editable} className="classic-highlight" onChange={edit('grade')} />
            .
          </p>
          ...
          <div className="classic-content-row">
            ...
            <div>
              <table className="classic-module-table">
                ...
                <tbody>
                  {data.moduleRows.map((row, index) => (
                    <tr key={`${row.no}-${index}`}>
                      <td>
                        <EditableField value={row.no} editable={editable} onChange={(value) => onEditModule?.(index, 'no', value)} />
                      </td>
                      <td>
                        <EditableField value={row.title} editable={editable} onChange={(value) => onEditModule?.(index, 'title', value)} />
                      </td>
                      <td>
                        <EditableField value={row.fullMarks} editable={editable} onChange={(value) => onEditModule?.(index, 'fullMarks', value)} />
                      </td>
                      <td>
                        <EditableField value={row.marksObtain} editable={editable} onChange={(value) => onEditModule?.(index, 'marksObtain', value)} />
                      </td>
                    </tr>
                  ))}
                  ...
                  <tr className="classic-total-row">
                    <td colSpan="2">
                      ...
                      Grade :{' '}
                      <EditableField value={data.grade} editable={editable} onChange={edit('grade')} />
                    </td>
              ...
              <div className="classic-issue">
                <EditableField value={data.issueDateText} editable={editable} onChange={edit('issueDateText')} />
                <br />
                Date of Issue
              </div>
            </div>
      ...
      <div className="classic-footer-strip">
        <span><EditableField value={data.officeAddress} editable={editable} onChange={edit('officeAddress')} /></span>
        <span><EditableField value={data.website} editable={editable} onChange={edit('website')} /></span>
      </div>
...
export default function Certificates() {
  ...
  const addModuleRow = () => {
    setEditor((prev) => {
      const moduleRows = [
        ...prev.moduleRows,
        { no: `${prev.moduleRows.length + 1}.`, title: 'New Module', fullMarks: '100', marksObtain: '80' }
      ];
      return withCalculatedMarks({ ...prev, moduleRows });
    });
  };
  ...
  const buildPayload = () => {
    ...
    const payload = {
      ...calculatedEditor,
      issueDate: calculatedEditor.issueDate?.toISOString ? calculatedEditor.issueDate.toISOString() : dayjs().toISOString(),
      remarks: calculatedEditor.duration,
      moduleRows: calculatedEditor.moduleRows.map((row) => ({
        no: row.no || '', title: row.title || '', fullMarks: row.fullMarks || '', marksObtain: row.marksObtain || ''
      }))
    };
    delete payload._id;
  ...
  const saveCertificate = async () => {
    if (!editor.studentName?.trim()) { message.error('Student name is required'); return; }
    if (!editor.courseTitle?.trim()) { message.error('Course title is required'); return; }

    try {
      const payload = buildPayload();
      const { data } = editingId ? await api.put(`/certificates/${editingId}`, payload) : await api.post('/certificates', payload);
  ...
  const printCertificate = async (targetRef = certRef) => {
    ...
    exportWrapper.style.cssText = `position: fixed; left: -100000px; top: 0; width: 1122px; height: 794px; background: #ffffff; overflow: hidden; z-index: -9999; opacity: 1; pointer-events: none;`;
    exportWrapper.innerHTML = `
      <style>
        ${CERTIFICATE_STYLES}
        .classic-certificate-wrap { width: 1122px !important; height: 794px !important; padding: 0 !important; margin: 0 !important; overflow: hidden !important; background: #ffffff !important; }
        .classic-certificate { width: 1122px !important; height: 794px !important; margin: 0 !important; border: 0 !important; box-shadow: none !important; transform: none !important; }
        .editable-cert-field { outline: 0 !important; }
      </style>
      <div class="classic-certificate-wrap">${targetRef.current.outerHTML}</div>
    `;
    ...
    try {
      ...
      const canvas = await html2canvas(certificateNode, { scale: 2.5, useCORS: true, allowTaint: true, backgroundColor: '#ffffff', width: 1122, height: 794, windowWidth: 1400, windowHeight: 900, scrollX: 0, scrollY: 0 });
      const imageData = canvas.toDataURL('image/jpeg', 1.0);
      const pdf = new jsPDF({ orientation: 'landscape', unit: 'mm', format: 'a4', compress: true });
      pdf.addImage(imageData, 'JPEG', 0, 0, 297, 210);
      const studentName = certificateNode.querySelector('.classic-highlight')?.textContent?.trim() || 'Student';
  ...
  const columns = [
    ...
    { title: 'Issue Date', dataIndex: 'issueDate', width: 145, render: (date) => dayjs(date).format('DD MMM YYYY') }
  ];
  ...
  if (isAdmin) {
    columns.push({
      ...
      render: (_, row) => (
        <Space>
          <Button icon={<FiAward />} onClick={() => setSelected(row)}>Preview</Button>
          <Button icon={<FiEdit3 />} onClick={() => openEditorFromCertificate(row)}>Edit</Button>
  ...
  return (
    <div>
      ...
      <PageHeader
        ...
        actionText={isAdmin ? "Generate Certificate" : undefined}
        actionIcon={isAdmin ? <FiPlus /> : undefined}
        onAction={isAdmin ? openNewEditor : undefined}
      />
      ...
      {selectedData && (
        <Card className="content-card" bordered={false}>
          ...
          <div
            ...
            title={isAdmin ? "Double click certificate to edit" : ""}
            onDoubleClick={isAdmin ? () => openEditorFromCertificate(selected) : undefined}
          >
        ...
        <div className="section-toolbar compact-toolbar">
          <Space wrap>
            <Input.Search
              ...
              onSearch={(value) => { setSearch(value); load(value); }}
        ...
        {loading ? (
          ...
          <Table
            ...
            onRow={(record) => ({
              ...
              onDoubleClick: isAdmin ? () => openEditorFromCertificate(record) : undefined,
            })}
          />
        )}
      </Card>

      <Modal title={editingId ? "Edit Certificate Like Document" : "Generate Dummy Certificate"} open={editorOpen} onCancel={() => setEditorOpen(false)} width="96vw" style={{ top: 18 }} footer={[<Button key="cancel" onClick={() => setEditorOpen(false)}>Cancel</Button>, <Button key="save" type="primary" icon={<FiSave />} onClick={saveCertificate}>{editingId ? "Update Certificate" : "Save Certificate"}</Button>]} destroyOnClose>
        <div className="certificate-editor-grid">
          <div className="certificate-editor-form">
            ...
            <Select value={editor.gender} onChange={(value) => updateEditor("gender", value)} options={[{ value: "Male" }, { value: "Female" }, { value: "Other" }]} style={{ width: "100%" }} />
            ...
            <Select value={editor.grade} onChange={(value) => updateEditor("grade", value)} options={[{ value: "A+" }, { value: "A" }, { value: "B+" }, { value: "B" }, { value: "C" }]} style={{ width: "100%" }} />
            <label className="editor-field-label">Issue Date</label>
            <DatePicker className="full-width" value={editor.issueDate} format="DD MMM YYYY" onChange={(value) => { updateEditor("issueDate", value || dayjs()); updateEditor("issueDateText", (value || dayjs()).format("DD/MM/YY")); }} />
            ...
            <Button block onClick={addModuleRow}>Add Module Row</Button>
          </div>
          <div className="classic-certificate-wrap">
            <ClassicCertificate ref={editorCertRef} data={editor} editable onEditField={updateEditor} onEditModule={updateModule} />
          </div>
        </div>
      </Modal>
    </div>
